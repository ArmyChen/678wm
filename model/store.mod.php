...
function store_fetch($id, $field = array())
{
	...
	$data2= pdo_fetch("SELECT ".$field_str." FROM ims_tiny_wmall_plus_store WHERE id = ".$id);

	$data1 = pdo_fetch("SELECT * FROM fanwe_supplier_location WHERE id = :id", array(':id' => $data2['slid']));
	...
	if($data['qualification']['business']['thumb']==""){
			$data['qualification']['business']['thumb']=$data['biz_license'];
	}
	if($data['qualification']['service']['thumb']==""){
			$data['qualification']['service']['thumb']=$data['biz_other_license'];
	}
	...
	if (isset($data['preview'])) {
		$checkstr=substr($data['preview'],0,2);
		if($checkstr=='./'){
		$data['preview']=substr($data['preview'],2,strlen($data['preview']));	
		}
		$data['preview'] = "http://www.678sh.com/".$data['preview'];
	}
	if (isset($data['logo'])) {
		$data['logo'] = tomedia($data['logo']);		
	}
	if($data['logo']!=''){
	$data['preview']=$data['logo'];
	}	
	if (isset($data['biz_license'])) {
		$checkstr=substr($data['biz_license'],0,2);
		if($checkstr=='./'){
		$data['biz_license']=substr($data['biz_license'],2,strlen($data['biz_license']));	
		}
		$data['biz_license'] = "http://www.678sh.com/".$data['biz_license'];
	}
	if (isset($data['biz_other_license'])) {
		$checkstr=substr($data['biz_other_license'],0,2);
		if($checkstr=='./'){
		$data['biz_other_license']=substr($data['biz_other_license'],2,strlen($data['biz_other_license']));	
		}
		$data['biz_other_license'] = "http://www.678sh.com/".$data['biz_other_license'];
	}
	if (isset($data['brief'])) {
		$data['brief'] = str_replace("src=\"./public/attachment/","src=\"http://www.678sh.com/public/attachment/",$data['brief']);
		$data['brief'] = str_replace("src=\"/public/attachment/","src=\"http://www.678sh.com/public/attachment/",$data['brief']);
	}
...
function store_check()
{
	...
	///////////增加店铺扫码外卖辅助数据库////////3.31///////////////////
	if($sid){
		if($_W['we7_wmall_plus']['store']['title']==""){
			  $slidname_info=pdo_fetch("select name from fanwe_supplier_location where id=".$sid);
			$_W['we7_wmall_plus']['store']['title']=$slidname_info['name'];
		}
	$insert = array(
	    'slid' => $sid,	
		...
		'is_reserve' => 1,
		'is_meal' => 1,
		'business_hours' => 'a:1:{i:0;a:2:{s:1:"s";s:5:"00:00";s:1:"e";s:5:"23:59";}}',
		'forward_mode' => 1
	);
	$slidinfo = pdo_get('tiny_wmall_plus_store', array('slid' => $sid));
	if(!$slidinfo){
	pdo_insert('tiny_wmall_plus_store', $insert);	
	}
	}
	///////////////////////////////////////////
	if (!defined('IN_MOBILE')) {
	...
	$store = pdo_fetch('SELECT a.id as slid,b.id, a.name,a.is_effect as status, b.pc_notice_status, b.delivery_mode FROM fanwe_supplier_location a left join ims_tiny_wmall_plus_store b on a.id=b.slid WHERE a.id ='.$sid);
	if (empty($store)) {
		if (!defined('IN_MOBILE')) {
			message('门店信息不存在或已删除1', '', 'error');
		}
		die;
	}
	
	$store['manager'] = pdo_fetchall('SELECT a.* FROM  fanwe_supplier_account a LEFT JOIN fanwe_supplier_location b on a.supplier_id=b.supplier_id LEFT JOIN fanwe_supplier_account_location_link c on c.location_id=b.id where a.id=c.account_id and b.id='.$sid);
	//$store['account'] = pdo_get('tiny_wmall_plus_store_account', array('sid' => $store['id']));
	//$_W['we7_wmall_plus']['store'] = $store;
	//var_dump($store['manager']);
	//var_dump($store);
...
function store_fetchall_category()
{
	...
	$data = pdo_fetchall('select id,name as title,icon_img as thumb from fanwe_deal_cate where is_effect=1 order by id ASC', array(':uniacid' => $_W['uniacid']), 'id');
	if (!empty($data)) {
		foreach ($data as &$da) {
			$da['thumb'] = str_replace("./public/","http://www.678sh.com/public/",$da['thumb']);
			$da['is_sys'] = 0;
			if (empty($da['link'])) {
				$da['is_sys'] = 1;
...
{
	global $_W;
	//2017-4-2 此处的store_ID为方维系统的location_id
	$condition = ' where location_id = :sid';
	$params = array(':sid' => $store_id);
	if ($status >= 0) {
		$condition .= ' and is_effect = :status';
		$params[':status'] = $status;
	}
	$data = pdo_fetchall('select *,name as title from fanwe_dc_supplier_menu_cate' . $condition . ' order by sort asc, id asc', $params, 'id');
	return $data;
}

...
function checkclerklogin() {
	global $_W;
	$s_account_info = json_decode(base64_decode($_COOKIE['57da___fanwe_cookie']),true);
	$clerk=array("id"=>$s_account_info['id'],"uniacid"=>7,"sid"=>$s_account_info['slid'],"role"=>'shanghu',
	"nickname"=> $s_account_info['account_name'],
    "mobile"=>$s_account_info['mobile'],
    "password"=>$s_account_info['account_name'],    
    "addtime"=>NOW_TIME);	
	$store=array("title"=>$s_account_info['slidname'],    
    "id"=>$s_account_info['slid']);	
	$we7_wmall_plus['clerk_id']=$s_account_info['id'];
	$we7_wmall_plus['sid']=$s_account_info['slid'];	
	$we7_wmall_plus['clerk']=$clerk;
	$we7_wmall_plus['store']=$store;
	$_W['we7_wmall_plus']=$we7_wmall_plus;
	$_W['uniacid']=7;
	//var_dump($_COOKIE);
...
function message($msg, $redirect = '', $type = '') {
	...
	$redirect=str_replace('&op=post','',$redirect);
	if($redirect == 'referer') {
		$redirect = referer();
	}
	if($redirect == '') {

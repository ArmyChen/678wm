...
class we7_wmall_plusModuleSite extends WeModuleSite
{
	...
	public function push_tuisong($params)
	{
	$dir = IA_ROOT . '/push/';	
	require $dir . 'hy_tool.class.php';
	$order_id=$params['order_id'];
	$sql="SELECT a.appid from fanwe_app a LEFT JOIN ims_tiny_wmall_plus_store b on a.slid=b.slid LEFT JOIN ims_tiny_wmall_plus_order c on c.sid=b.id where c.id=:order_id and isdisable=1 ORDER BY a.loginTime desc limit 1";
	$channelId = pdo_fetch($sql,array("order_id"=>$order_id));
	$name=array($channelId['appid']);		
	$type = "cmd";
	$description=array('code'=>$params['code'],'data'=>array('order_id'=>$order_id));
	    $message = array (    
 		'title' => '提示',
        'description' =>'你有新的订单' ,
	...
	$ht = new HyTool ();
    $ht->sendMessage ($name,$type,$message);
	file_put_contents("abc_push.txt",$channelId.'推送成功');
	}
	public function payResult($params)
	{
	    ...
	    file_put_contents("abc.txt","支付成功！");
		//2017-4-5 推送代码加在这个地方
		$new_push_array=array("order_id"=>$record['order_id'],"code"=>'7401'); 
		$this->push_tuisong($new_push_array);		
		}

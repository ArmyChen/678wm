...
function order_clerk_notice($id, $type, $note = '')
{
	...
	//$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'order', 'm' => 'we7_wmall_plus', 'op' => 'detail', 'sid' => $order['sid'], 'id' => $order['id'])), '.');
	$url = 'http://wm.678sh.com/app' . ltrim(murl('entry', array('do' => 'order', 'm' => 'we7_wmall_plus', 'op' => 'detail', 'sid' => $order['sid'], 'id' => $order['id'])), '.');
	$remark = implode("\n", $remark);
	$send = tpl_format($title, $order['ordersn'], $order['status_cn'], $remark);
	mload()->model('sms');
...
{
	$pay_types = array('' => '未支付', 'alipay' => array('css' => 'label label-info', 'text' => '支付宝'), 'wechat' => array('css' => 'label label-success', 'text' => '微信支付'),  'bestpay' => array('css' => 'label label-warning', 'text' => '翼支付'),'credit' => array('css' => 'label label-warning', 'text' => '余额支付'), 'delivery' => array('css' => 'label label-primary', 'text' => '吧台支付'), 'baifubao' => array('css' => 'label label-primary', 'text' => '百付宝支付'), 'cash' => array('css' => 'label label-primary', 'text' => '现金支付'));
	return $pay_types;
}
function order_insert_member_cart($slid,$sid, $ignore_bargain = false)
{
	global $_W, $_GPC;
	...
	if (!empty($_GPC['goods'])) {
		...
		$goods_info = pdo_fetchall("SELECT *,name as title FROM fanwe_dc_menu WHERE is_effect_enable=1 and location_id = :sid AND id IN ({$ids_str})", array(':sid' => $slid), 'id');
		//$goods_info = pdo_fetchall("SELECT *,name as title FROM fanwe_dc_menu WHERE location_id = :sid)", array(':sid' => $slid), 'id');
		if (!$ignore_bargain) {
			mload()->model('bargain');
		...
		foreach ($_GPC['goods'] as $k => $v) {
			...
			if (!$opts['is_options']) {
				...
				if ($num > 0) {
                  ...
                  $cart_data_json[$k][]=$cart_item; //2017-4-24传值给JSON串
				}
				//echo $k."FORE::::::::::::::";
              //  var_dump($cart_data_json);
...
//						}
                        ...
                        $cart_data_json[$k][] = array('title' => $goods_info[$k]['title'] . "({$val['name']})", 'num' => $val['num'], 'price' => $val['price_total'], 'discount_price' => $val['price_total'], 'discount_num' => 0, 'price_num' => $num, 'total_price' => round($val['price_total'] * $val['num'], 2), 'total_discount_price' => round($val['price_total'] * $val['num'], 2), 'bargain_id' => 0);
						//$cart_goods[$key] = array('title' => $goods_info[$k]['title'] . "({$val['name']})", 'num' => $val['num'], 'price' => $val['price_total'], 'discount_price' => $val['price_total'], 'discount_num' => 0, 'price_num' => $num, 'total_price' => round($val['price_total'] * $val['num'], 2), 'total_discount_price' => round($val['price_total'] * $val['num'], 2), 'bargain_id' => 0);
                        //var_dump($cart_data_json);
						$total_num += $val['num'];
		...
		$isexist = pdo_fetchcolumn('SELECT id FROM ' . tablename('tiny_wmall_plus_order_cart') . " WHERE uniacid = :aid AND sid = :sid AND uid = :uid", array(':aid' => $_W['uniacid'], ':sid' => $sid, ':uid' => $_W['member']['uid']));
		$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid'], 'groupid' => $_W['member']['groupid'], 'num' => $total_num, 'price' => $total_price, 'box_price' => round($total_box_price, 2), 'data' => iserializer($cart_goods),'data_json' => iserializer($cart_data_json), 'original_data' => iserializer($_GPC['goods']), 'addtime' => TIMESTAMP, 'bargain_use_limit' => 0);
...
{
	global $_W;	
	$slid=pdo_get('tiny_wmall_plus_store',array("id"=>$sid),array("slid"));$slid=intval($slid['slid']);	
	...
	$goods_info = pdo_fetchall("SELECT *,name as title,stock as total FROM fanwe_dc_menu WHERE location_id = :sid AND id IN ({$goods_ids})", array(':sid' => $slid), 'id');
...
function order_insert_cart($sid)
{
	...
	if (!empty($_GPC['goods'])) {
		...
		$box_price = 0;
		$ids_str = implode(',', array_keys($_GPC['goods']));
		$goods_info = pdo_fetchall("SELECT *,name as title FROM fanwe_dc_menu WHERE location_id = :sid AND id IN ({$ids_str})", array(':sid' => $slid), 'id');
		foreach ($_GPC['goods'] as $k => $v) {
			$k = intval($k);
...
function order_update_goods_info($order_id, $sid, $cart = array())
{
	...
	$goods_info = pdo_fetchall("SELECT id,cate_id as cid,name as title,price,stock as total,print as total_update_type,print_label FROM fanwe_dc_menu WHERE location_id = :sid AND id IN ({$ids_str})", array(':sid' => $slid), 'id');
	foreach ($cart['data'] as $goods_id => $options) {
		...
		foreach ($options as $option_id => $item) {
			if ($goods['total_update_type'] == 3) {  //外购商品直接减库存
				if (!$option_id) {
					if ($goods['total'] != -1 && $goods['total'] > 0) {
						pdo_query("UPDATE fanwe_dc_menu set stock = stock - {$item['num']} WHERE id = :id", array(':id' => $goods_id));
					}
					if ($item['bargain_id'] > 0 && $item['discount_num'] > 0) {
...
function order_system_status_update($id, $type, $extra = array())
{
	...
	if ($type == 'pay') {
		...
		if (!empty($stat)) {
			foreach ($stat as $row) {
				pdo_query("UPDATE fanwe_dc_menu set buy_count = buy_count + {$row['goods_num']} WHERE  id = :id", array(':id' => $row['goods_id']));
				...
				if (!$row['total_update_status']) {
					if (!$row['option_id']) {
						$goods = pdo_fetch("select * from fanwe_dc_menu where id=:id", array('id' => $row['goods_id']));
						if ($goods['print'] == 3 ) {  //外购商品直接减库存
							pdo_query("UPDATE fanwe_dc_menu set stock = stock - {$row['goods_num']} WHERE id = :id", array(':id' => $row['goods_id']));
						}

...
function goods_avaliable_fetchall($slid,$sid, $cid = 0, $ignore_bargain = false)
{
	...
	$goods = pdo_fetchall('select id,cate_id as cid,name as title, price,customerPrice as box_price, stock as total,image as thumb, sailed, info as label, m_desc as content, is_options, unit as unitname, comment_good,print from fanwe_dc_menu' . $condition, $params, 'id');
	...
	unset($options);
	$condition = " where is_effect_enable=1 and uniacid = :uniacid and sid = :sid and status = :status order by id limit 2";
	...
	foreach ($goods as &$good) {
	    if($good['print']==1){$good['total']=1000;}  //设置现制商品无限库存
        ...
        foreach($goods_tastes as &$goods_taste){
          //  file_put_contents("s.txt", $good['id'].$goods_taste['shops'].PHP_EOL,FILE_APPEND);

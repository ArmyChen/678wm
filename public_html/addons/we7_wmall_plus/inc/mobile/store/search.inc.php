...
if($op == 'list') {
	...
	$stores = pdo_fetchall('select id,slid,title,logo,label,sailed,score,business_hours,is_in_business,delivery_fee_mode,delivery_price,delivery_free_price,send_price,delivery_time,delivery_mode,token_status,invoice_status,location_x,location_y,forward_mode,forward_url from ' . tablename('tiny_wmall_plus_store') . $condition, $params);
	$min = 0;
	if(!empty($stores)) {
		...
		foreach($stores as &$row) {
			...
			$row['sailed']= pdo_fetchcolumn('select sum(buy_count) from fanwe_dc_menu where location_id = :sid ', array(':sid' => $row['slid']));

...
if($op == 'list') {
	...
	$stores = pdo_fetchall('select id,slid,score,title,logo,sailed,score,label,business_hours,is_in_business,delivery_fee_mode,delivery_price,delivery_free_price,send_price,delivery_time,delivery_mode,token_status,invoice_status,location_x,location_y,forward_mode,forward_url,displayorder,click from ' . tablename('tiny_wmall_plus_store') . " where uniacid = :uniacid and status = 1", array(':uniacid' => $_W['uniacid']));
	$min = 0;
	if(!empty($stores)) {
		...
		foreach($stores as &$da) {
			...
			$da['sailed']= pdo_fetchcolumn('select sum(buy_count) from fanwe_dc_menu where location_id = :sid ', array(':sid' => $da['slid']));

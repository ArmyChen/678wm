...
if($op == 'index') {
	...
	$result = goods_avaliable_fetchall($slid,$sid);
...
if($op == 'cate') {
	$cart = order_insert_member_cart($slid,$sid);
	$cid = trim($_GPC['cid']);
	$result = goods_avaliable_fetchall($slid,$sid, $cid);
	$categorys = $result['category'];
	$cate_goods = $result['cate_goods'];
	$goods = $result['goods'];
	$bargains = $result['bargains'];
...
if($op == 'search') {
	...
	$goods = pdo_fetchall('select * from fanwe_dc_menu where is_effect_enable=1 and location_id = :sid and is_effect = 1 and name like :title', array(':sid' => $slid, ':title' => "%{$key}%"));
	if(!empty($goods)) {
		foreach($goods as &$good) {
			$good['thumb_cn'] = str_replace("./public/attachment/","http://www.678sh.com/public/attachment/",$good['image']);
			$good['thumb'] = str_replace("./public/attachment/","http://www.678sh.com/public/attachment/",$good['image']);
			$good['title']=$good['name'];
			$good['sailed']=$good['buy_count'];
			$good['unitname']=$good['unit'];
			$good['label']=$good['info'];

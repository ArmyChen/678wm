...
if($op == 'mine') {
	...
	$detail_id=intval($_GPC['detail_id']);
    if($detail_id){	
	$mine = pdo_get('tiny_wmall_plus_assign_board', array('id' => $detail_id));	
	}else{
	$mine = pdo_get('tiny_wmall_plus_assign_board', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'uid' => $_W['member']['uid'], 'status' => 1));
	}
	...
	$now_number = pdo_fetchcolumn('select number from ' . tablename('tiny_wmall_plus_assign_board') . ' where uniacid = :uniacid and sid = :sid and status = 1 and queue_id = :queue_id and  id < :id order by id asc', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':queue_id' => $mine['queue_id'], ':id' => $mine['id']));
	if(empty($now_number)) {

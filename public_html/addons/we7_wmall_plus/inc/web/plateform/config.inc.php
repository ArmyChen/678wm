...
if($op == 'pay') {
	...
	if(checksubmit('submit')) {
		$payment = array(
			...
			'bestpay' => intval($_GPC['bestpay']),
			'credit' => intval($_GPC['credit']),
			'delivery' => intval($_GPC['delivery']),
			'wechat_cert' => $config['payment']['wechat_cert'],
			'alipay_cert' => $config['payment']['alipay_cert'],
			'bestpay_appid' => array("app_id"=>$_GPC['bestpay_appid'],"trade_key"=>$_GPC['trade_key'],"data_key"=>$_GPC['data_key']),
		);
		$data = array(
			'uniacid' => $_W['uniacid'],
		...
		foreach($payment as $key => $row) {
			if(in_array($key, array('credit', 'wechat', 'alipay','bestpay', 'delivery')) && $row == 1) {

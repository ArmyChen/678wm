...
if($op == 'build') {
	...
	*/
	...
	$types = array(
        ...
        'assign_id' => array(
            'scene_str' => "we7_wmall_plus__assign_{$sid}_{$sign_id}",
            'keyword' => "we7_wmall_plus__assign_{$sid}_{$sign_id}",
            'name' => "{$store['title']}__{$sign_id}号二维码",
        ),
		'table' => array(
    ...
    }elseif($type == 'assign_id'){ //APP排号2维码
        //http://wm.678sh.com/addons/we7_wmall_plus/admin/index.php?c=site&a=entry&op=build&store_id=1&sign_id=167&type=assign_id&do=ptfqrcode&m=we7_wmall_plus
        //以上链接返加个链接
        // 二维码：http://wm.678sh.com/web/index.php?c=utility&a=wxcode&do=qrcode&text= 以上的链接URLCODE后显示
        echo $result['url'];
        exit;
	} elseif($type == 'table') {

...
$id = intval($_GPC['id']);  //返回的有STROE——ID
if($op!='list'){
    $store=store_check();
}else{
...
if($op == 'post') {
	...
	$slid=intval($_GPC['slid']);
    if($slid){
    $slidname_info=pdo_fetch("select name from fanwe_supplier_location where id=".$slid);
	//file_put_contents("a.txt",json_encode($slidname_info));
	$insert = array(
	    'slid' => $slid,	
	    'title' => $slidname_info['name'],	
		'uniacid' => $_W['uniacid'],	
		'is_assign' => 1,
		'is_reserve' => 1,
		'is_meal' => 1,
	...
	$slidinfo = pdo_get('tiny_wmall_plus_store', array('slid' => $slid));
	if(!$slidinfo){
	...
	if(checksubmit('submit')) {
		...
		if($id) {			
			pdo_update('tiny_wmall_plus_store', $data, array('id' => $id));
			$sid = $id;
		} else {
			...
			pdo_update('tiny_wmall_plus_store', $update, array('id' => $sid));
		}
		store_delivery_times($sid, true);
		message('编辑门店信息成功', $this->createWebUrl('store', array('op' => 'post','id'=>$id)), 'success');
...
if($op == 'list') {
	...
	$cid = intval($_GPC['cid']);
	if($cid > 0) {
		$condition .= " AND deal_cate_id LIKE :cid";
		$params[':deal_cate_id'] = "%|{$cid}|%";
	}
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM fanwe_supplier_location  ' . $condition, $params);
	$lists = pdo_fetchall('SELECT * FROM fanwe_supplier_location ' . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * $psize.','.$psize, $params);
	$pager = pagination($total, $pindex, $psize);
	if(!empty($lists)) {
		foreach($lists as &$li) {
			$li['cid'] = $li['deal_cate_id'];
			$li['title'] = $li['name'];
			$li['cid'] = intval($li['deal_cate_id']);
			$li['logo'] =str_replace("./public/attachment/","http://www.678sh.com/public/attachment/",$li['preview']);			
			...
			//li['sid'] = pdo_get('tiny_wmall_plus_store', array('slid' => intval($li['id'])),array("id"));
		}
	}
	$categorys = store_fetchall_category();

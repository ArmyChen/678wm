...
class we7_wmall_plusModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		if($row['type'] == 'store') {
            ...
            return $this->respNews($news);
        }elseif($row['type'] == 'assign_id') {
            //APP 排号二维码
            if (!$store['is_assign']) {
                return $this->respText("{$store['title']} 已关闭排号功能,请联系商家");
            }
            //创建会员
//            微信会员ID： $_W['member']['uid']
//            $_W['openid']

           //更新当前排号的状态

            //排队情况
            $table = pdo_get('tiny_wmall_plus_assign_board', array('uniacid' => $_W['uniacid'], 'id' => $row['table_id']));
            if(empty($table)) {
                return '';
            }elseif($table['uid']==0){
            pdo_update('tiny_wmall_plus_assign_board', array('uid' => $_W['member']['uid'],'openid'=>$_W['openid']), array('id' => $row['table_id']));
            }
            $url = murl('entry', array('m' => 'we7_wmall_plus', 'do' => 'assign', 'sid' => $sid,'op'=>'mine'), true, true);
            $news = array();
            $news[] = array(
                'title' => $store['title'] . "-点击查看最新排号状态",
                'description' => $store['content'],
                'picurl' => $store['logo'],
                'url' => $url
            );

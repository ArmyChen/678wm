...
$store = store_fetch($store['id']);
$do = 'order';
$op = trim($_GPC['op']) ? trim($_GPC['op']) : 'list';

if($op == 'list') {
	$condition = ' WHERE 1 = 1 AND sid = :sid and order_type < 3';
	...
	$wait_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tiny_wmall_plus_order') . ' WHERE sid = :sid and status = 1 and order_type < 3', array(':sid' => $sid));
